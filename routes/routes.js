...
var router = (app) => {
    ...
    app.route('/api/todos').get((req, res) => {
        app.get('database').collection('todos').find({'user':req.session.user.username}).toArray((err, result) => {
    ...
    app.route('/api/todos/update/:id').post((req, res) => {
        app.get('database').collection('todos').updateOne({ '_id': new ObjectId(req.params.id) },
    ...
    app.route('/api/todos/delete/:id').delete((req, res) => {
        app.get('database').collection('todos').deleteOne({ '_id': new ObjectId(req.params.id) }, 
            (err, result) => {
                if (err) {
                    console.log(err)
                }
    ...
    app.route('/api/todos/share').post((req,res) => {
        app.get('database').collection('todos').updateOne({ '_id': new ObjectId(req.body.id) }, 
            {$push : {'shared': req.body.username}}, (err, docs) => {
                if(err) {
                    res.json(err)
                } else {
    ...
    app.route('/api/todos/getSharedTodos').get((req,res) => {
        app.get('database').collection('todos').find({ 'shared':req.session.user.username }).toArray((err,docs) => {
            if(err) {
                res.json(err)
            } else {
                res.json(docs)
